...
export class MongoRepository implements DepotRepository {
    ...
    public async insertDepot(depot: DepotValue): Promise<any | null> {
        const insertedDepot = await DepotModel.default.create(depot);
        return insertedDepot;
    }

    public async getDepots(seller_id): Promise<DepotValue[] | null> {
        const depots: DepotValue[] = await DepotModel.default.find({ "seller_id": seller_id }, { _id: 1, id: 1, name: 1, city: 1, state: 1, address: 1 });
        return depots;
    }

    public async updateDepot({ id, seller_id, name, state, city, address }: DepotValue): Promise<any | null> {

        const updatedDepot = await DepotModel.default.updateOne({ "id": `${id}` }, { seller_id, name, state, city, address });
        return updatedDepot;
    }

    public async deleteDepot(id: string): Promise<any | null> {
        const deletedDepot = await DepotModel.default.deleteOne({ "id": id });
        return deletedDepot;
    }
    ...
    public async allDepots(pag: number): Promise<any> {
        ...
        var result = await DepotModel.default.paginate({}, options);
